    ...
    >
      <Box as="a" href={href} _hover={{ textDecor: "none" }} role="group">
        <Box p="0" height="full" borderRadius="8px" bg="card-bg">
          ...
          >
            <Flex
              gap={type === "featured" ? "1" : "8"}
              direction={type === "featured" ? "row" : "column"}
              flex={1}
            >
              {children}
            </Flex>
...
const Image = ({ url, imageAlt, type = "grid" }: ImageProps) => {
  ...
  const featuredImageSize = useBreakpointValue({
    base: "581px",
    sm: "350px",
    md: "430px",
    lg: "550px",
  ...
  const cloudflareImage = `https://starknet.io/cdn-cgi/image/width=${
    type === "featured" ? featuredImageSize : size
  },height=auto,format=auto${url}`;
