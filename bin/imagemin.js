...
function compressImage(filePath, quality) {
  if (quality < 50) {
    logger.warn(`Minimum quality reached but file size is still above 2MB for: ${filePath}`);
    logger.info(divider);
...
fs.readdir(directory, (err, files) => {
  ...
  files.forEach(file => {
    compressImage(path.join(directory, file), 80);
  });
});